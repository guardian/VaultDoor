...
describe("VaultSelector", () => {
  ...
  it("should render a dropdown of all vaults", (done) => {
    ...
    const rendered = shallow(
      <VaultSelector currentvault={""} vaultWasChanged={vaultChangedCb} />
    );
    ...
    setTimeout(() => {
      try {
        ...
        expect(selector.children("option").at(1).prop("value")).toEqual(
          "fake-id-1"
        );
        expect(selector.children("option").at(0).text()).toEqual("Vault 2");
        expect(selector.children("option").at(0).prop("value")).toEqual(
          "fake-id-2"
        );
        done();
      } catch (err) {
