...
describe("refreshLogin", () => {
  ...
  it("should call out to the IdP endpoint with the refresh token and resolve on success", async () => {
    ...
    expect(localStorage.getItem("vaultdoor:access-token")).toEqual(
      "some-access-token"
    );
    expect(localStorage.getItem("vaultdoor:refresh-token")).toEqual(
  ...
  it("should reject with Request forbidden if the server returns a 403", async () => {
    fetchMock.mockReturnValue(
      Promise.resolve(
        new Response(JSON.stringify({ status: "forbidden" }), { status: 403 })
      )
    );
  ...
  it("should reject if the server returns an unexpected code", async () => {
    fetchMock.mockReturnValue(
      Promise.resolve(
        new Response(JSON.stringify({ status: "coffee" }), { status: 418 })
      )
    );
  ...
  it("should retry on a 503 or 500 error", async () => {
    fetchMock.mockResponses(
      ["", { status: 503 }],
      [JSON.stringify({ status: "broken" }), { status: 500 }],
      [
        JSON.stringify({
          access_token: "some-access-token",
          refresh_token: "some-new-refresh-token",
