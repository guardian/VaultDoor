  ...
  async setupCurrentSearch() {
    ...
    return new Promise((resolve, reject) => {
      if (searchParams.hasOwnProperty("project")) {
        this.setState({ currentProjectSearch: searchParams.project }, () =>
          resolve()
        );
      } else {
        resolve();
  ...
  async loadFrontendConfig() {
    ...
    if (response.ok) {
      ...
      return new Promise((resolve, reject) => {
        this.setState(
          {
            projectLockerBaseUrl: content.projectLockerBaseUrl,
            plutoBaseUrl: content.plutoBaseUrl,
          },
          () => resolve()
        );
      });
      ...
      <div className="windowpanel">
        <div className="search-bar-element">
          <NewVaultSelector
            ...
            vaultWasChanged={(newVaultId) =>
              this.setState({ vaultId: newVaultId })
            }
          />
        </div>
        <ProjectLockerSearchBar
          projectLockerBaseUrl={this.state.projectLockerBaseUrl}
          projectSelectionChanged={(newProject) =>
            this.setState({ currentProjectSearch: newProject })
          }
          size={8}
        />
        <ProjectContentSummary
          vaultId={this.state.vaultId}
          projectId={this.state.currentProjectSearch}
          plutoBaseUrl={this.state.plutoBaseUrl}
        />
      </div>
    );
  }
