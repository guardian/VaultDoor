...
const PathView:React.FC<PathViewProps> = (props) => {
    const truncateParts = () => {
        const pathParts = props.stripStart ? props.pathParts.slice(props.stripStart) : props.pathParts;
        ...
        switch(props.truncateMode){
            ...
            default:
                const sectionLength = props.limit ? props.limit / 2 : pathParts.length-1;
                const firstPart = pathParts.slice(0, sectionLength+1);
                const lastPart = ["..."].concat(props.pathParts.slice(props.pathParts.length - sectionLength +1));
