...
class VaultSelector extends React.Component {
        ...
        switch(response.status) {
            case 200:
                ...
                if(Array.isArray(content)) {
                    ...
                    if (this.props.currentvault === "") this.props.vaultWasChanged(reversed[0].vaultId);
                } else {
    ...
    render() {
        return <span className="form-spacing">
            {this.state.lastError ? <p className="error">{this.state.lastError}</p> : <>
                <label htmlFor="vaultsDropdown">Select vault: </label>
                <select id="vaultsDropdown" value={this.props.currentvault}
                        onChange={evt => this.props.vaultWasChanged(evt.target.value)}>
                    {this.state.knownVaults.map(entry => <option value={entry.vaultId}>{entry.name}</option>)}
                </select>
            </>
            }
        </span>
