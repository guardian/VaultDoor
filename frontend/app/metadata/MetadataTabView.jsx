...
class MetadataTabView extends React.Component {
    ...
    tableContent() {
        if(this.state.selectedTabIndex>=this.props.tabPrefixes.length){
            console.error("Requested tab " + this.state.selectedTabIndex + " but there are only " + this.props.tabPrefixes.length + " categories");
            return <table><tbody><tr><td><i>Not properly configured</i></td></tr></tbody></table>;
        }
        const metadata = this.breakdownMetadataString();

        return <table className="metadata-view-table">
            {
                Object.keys(metadata)
                    .sort()
                    .filter(key=>key.startsWith(this.props.tabPrefixes[this.state.selectedTabIndex]))
                    .map((key,idx)=><tr key={idx}>
    ...
    render() {
        return <div className="metadata-view-container">
            <span className="tab-header-row">
                {
                    this.props.tabNames.map(
                        (name,idx)=>
                            <span
                                className={"tab-header" + (idx===this.props.selectedTabIndex ? " selected" : "")}
