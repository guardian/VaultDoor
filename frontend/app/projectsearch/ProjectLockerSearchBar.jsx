...
class ProjectLockerSearchBar extends React.Component {
    ...
    async initialWorkingGroupLoad() {
        if(this.props.projectLockerBaseUrl==="") return;
        const response = await(fetch(this.props.projectLockerBaseUrl + this.workingGroupFetchUrl, {credentials: "include"}));
        const bodyText = await response.text();

        if(response.ok){
            return new Promise((resolve, reject)=>{
                const bodyContent = JSON.parse(bodyText);
                this.setState({knownWorkingGroups: ProjectLockerSearchBar.workingGroupContentConverter(bodyContent)}, ()=>resolve())
            })
        } else {
            return new Promise((resolve, reject)=>{
    ...
    workingGroupChanged(newWg){
        ...
        this.setState({currentWorkingGroup: newWg, commSearchCounter: this.state.commSearchCounter+1, currentCommissionVsid: "", projSearchCounter: this.state.projSearchCounter+1});
    }

    static workingGroupContentConverter(incomingData){
        console.log("workingGroupContentConverter: ", incomingData);
        return incomingData.result.map(entry=>{return {name: entry.name, value: entry.id}})
    }
    ...
    static commissionContentConverter(incomingData) {
        ...
        return incomingData.result.map(entry => {
            return {name: entry.title, value: entry.siteId + "-" + entry.collectionId.toString()}
        })
    }

    static projectContentConverter(incomingData){
        console.log("projectContentConverter: ", incomingData);
        return incomingData.result.map(entry=>{return {name: entry.title, value: entry.vidispineId}})
    }
    ...
    render() {
        ...
        return <span>
            ...
            <div className="search-bar-element">
                <h3>Commission</h3>
                <FilterableList onChange={updatedComm=>this.setState({currentCommissionVsid: updatedComm, projSearchCounter: this.state.projSearchCounter+1})}
