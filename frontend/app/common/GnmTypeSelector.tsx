...
const GnmTypeSelector: React.FC<GnmTypeSelectorProps> = (props) => {
  ...
  useEffect(() => {
    loadData().catch((err) => {
      ...
      if (props.onError) {
        props.onError(
          `Could not load in gnm type values, see console for details`
        );
      }
    });
  }, []);
  ...
  const loadData = async () => {
    ...
    if (response.status == 200) {
      ...
      console.error(
        `Could not load in gnm type values: server error ${response.status} (${content})`
      );
      if (props.onError) {
        props.onError(
          `Could not load in gnm type values: server error ${response.status}`
        );
      }
    }
  };
  ...
  return (
    ...
    >
      ...
      {knownTypes.map((t, idx) => (
        <MenuItem key={idx} value={t} className={classes.menuText}>
          {t}
        </MenuItem>
      ))}
    </Select>
